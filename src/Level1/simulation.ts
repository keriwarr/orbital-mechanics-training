...
const length = (v: Vector) => Math.sqrt(v.x ** 2 + v.y ** 2);
  ...
  return makeCancelableBatchSimulation({
    simulateOneTick: ({ tick }) => {
      ...
      const getGravity = (p: Vector) => {
        const distance = sub(planetPosn, p);

        const distanceFromPlanetCenter = length(distance);

        const effectiveGravity =
          planetGravity * (planetRadius / distanceFromPlanetCenter) ** 2;

        if (distance.y === 0) return { x: effectiveGravity, y: 0 };

        const distanceRatio = distance.x / distance.y;

        const unsignedgravityy =
          effectiveGravity / Math.sqrt(distanceRatio ** 2 + 1);
        const unsignedgravityx = distanceRatio * unsignedgravityy;
        return {
          x: Math.sign(distance.x) * Math.abs(unsignedgravityx),
          y: Math.sign(distance.y) * Math.abs(unsignedgravityy),
        };
      };

      const getAccel = (p: Vector) => {
        const gravity = getGravity(p);

        return add(thrust, gravity);
      };
      ...
      velo = add(velo, scale(accel, 1 / TICK_PER_SECOND));
